...
TimeNotesServer 是 TimeNotes 的多人协作服务端。它负责创建协作房间、校验房间密钥、保存 Yjs 文档状态、转发 WebRTC 信令，并在 P2P DataChannel 不可用时提供服务器中转。
...
4. 服务端返回 `wsUrl` 和邀请链接，邀请链接把 `roomKey` 放在 URL fragment。
5. 协作者通过邀请链接加入，客户端连接 `GET /ws/collab`。
6. WebSocket 首帧必须是 `auth`，校验通过后服务端返回 `auth_ok`，其中包含在线成员和 SQLite 中保存的 Yjs 状态。
7. 第一位加入房间的用户成为房主，后续加入者是协作者。
8. 客户端之间优先建立 WebRTC DataChannel：
   - `doc`：可靠有序，传输 Yjs update。
   - `presence`：低延迟，传输鼠标、当前页面、选中元素、正在编辑元素。
   - `chat`：聊天消息。
9. P2P 失败、断开或强制中转时，同一 envelope 通过服务器 relay 广播或定向投递。
10. 房主退出时服务端广播 `room_closed`，其他协作者自动退出，房间被持久标记为关闭，旧邀请链接失效。
...
11. 准备 STUN/TURN。P2P 在公网 NAT 下不保证成功；生产推荐配置 TURN，避免所有实时流量都压到应用服务器 relay。
